  <footer class="site-footer">
    <div class="container">
      <div class="footer-grid">
        <div class="footer-brand">
          <a href="<?php echo e(url('/')); ?>" class="footer-logo">
            <img src="<?php echo e(asset('public/assets/logo.png')); ?>" alt="Pouch Gallery" class="footer-logo-img" />
            <span>POUCH GALLERY<sup>®</sup></span>
          </a>
          <p>Your one-stop shop for premium gaming peripherals, bags, and tech accessories. Trusted by 50,000+ customers across India.</p>
          <div class="social-links">
            <a href="#" aria-label="Instagram"><i class="ri-instagram-line"></i></a>
            <a href="#" aria-label="Facebook"><i class="ri-facebook-line"></i></a>
            <a href="#" aria-label="YouTube"><i class="ri-youtube-line"></i></a>
            <a href="#" aria-label="Twitter"><i class="ri-twitter-x-line"></i></a>
          </div>
        </div>
        <div class="footer-col">
          <h4>All Categories</h4>
          <?php $__currentLoopData = $catlimit; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fcat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(url('gaming-products/'.$fcat->id)); ?>"><?php echo e($fcat->category_name); ?></a>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          
        </div>
        <div class="footer-col">
          <h4>Gaming</h4>
        
          <a href="#">Track Order</a>
          <a href="#">Returns & Exchange</a>
          <a href="#">Shipping Policy</a>
          <a href="#">Warranty Info</a>
          <a href="#">FAQ</a>
          <a href="#">Contact Support</a>
        </div>
        <div class="footer-col">
          <h4>Contact Us</h4>
          <div class="footer-contact">
            <i class="ri-map-pin-line"></i>
            <span>123, Tech Hub, MG Road, Kerala – 680001</span>
          </div>
          <div class="footer-contact">
            <i class="ri-phone-line"></i>
            <span>+91 98765 43210</span>
          </div>
          <div class="footer-contact">
            <i class="ri-mail-line"></i>
            <span>support@pouchgallery.in</span>
          </div>
          <div class="footer-contact">
            <i class="ri-time-line"></i>
            <span>Mon–Sat: 9 AM – 7 PM</span>
          </div>
        </div>
      </div>
      <div class="footer-bottom">
        <p>© 2026 Pouch Gallery®. All rights reserved.</p>
        <div class="footer-links">
          <a href="#">Privacy Policy</a>
          <a href="#">Terms of Service</a>
          <a href="#">Sitemap</a>
        </div>
      </div>
    </div>
  </footer>

  <div class="toast" id="cartToast">
    <i class="ri-shopping-cart-line"></i>
    <span id="toastMsg">Added to cart!</span>
  </div>

  <button class="back-top" id="backTop" aria-label="Back to top">
    <i class="ri-arrow-up-line"></i>
  </button>
<?php /**PATH C:\xampp\htdocs\ebook\resources\views/layouts/webpartials/footer.blade.php ENDPATH**/ ?>